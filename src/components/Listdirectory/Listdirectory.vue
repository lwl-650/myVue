      ...
      <div class="other">
        ...
        <div class="other_b">
          <div class="getTime">
            ...
            <div>
              第{{ lottery_no }}期(<span style="color: red">当期</span>)
            </div>
          </div>
          <div class="ssq">
          ...
          <div class="tuissq">
            <div
              ...
              "
            >
              当期推荐红球{{ beforBall.length }}码：
            </div>
            <div class="tui_ball" v-for="item in beforBall" :key="item">
              {{ item }}
            </div>
          </div>
          <div class="tuiTime">
            <div>
              第{{ lottery_no + 1 }}期(<span style="color: red">下期</span>)
            </div>
          </div>
          <div class="tuissq">
            <div
              ...
              "
            >
              推荐红球{{ redBall.length }}码：
            </div>
            <div class="tui_ball" v-for="item in redBall" :key="item">
              {{ item }}
            </div>
          </div>
          <div class="gossq" @click="goX_ssq">去选号</div>
        </div>
      </div>
    </div>
...
import { ssq, getStu, prot } from "@/http/api";
export default defineComponent({
  ...
  setup() {
    ...
    const methods = {
      ...
      getProt() {
        prot().then((res) => {
          // console.log(res.data.prot);
          data.prot=res.data.prot
        });
      },
      getStu() {
        getStu().then((res) => {
      ...
      getssq(a, b, c, d, e, f) {
        ...
        let allArr = [
          ...new Set(
            [...arr1, ...arr2, ...arr3, ...arr4].sort((n1, n2) => n1 - n2)
          ),
        ];
        return allArr;
